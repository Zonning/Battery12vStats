...
double Battery12vStats::getBatteryVolts() {
  int readValue = _avgAnalogRead(_adcPin, _reads);
  return _analogReadToVolts(readValue);
}

int Battery12vStats::getBatteryChargeLevel(bool useConversionTable) {
  int readValue = _avgAnalogRead(_adcPin, _reads);
...
int Battery12vStats::_avgAnalogRead(int pinNumber, int reads) {
  int totalValue = 0;
...
int Battery12vStats::_calculateChargeLevel(double volts) {
  if (volts <= 3.700) {
    return (20 * volts) - 64;
  }
...
int Battery12vStats::_getChargeLevelFromConversionTable(double volts) {
